...
# Configuration - Official Vulkan version
MODELS = [
...
BACKENDS = [
    {"name": "Vulkan-Official", "exe": r"D:\onedrive\codelab\Gemini-Subtitle-Pro\MelBandRoformer.cpp-official\build-vulkan\Release\bs_roformer-cli.exe"}
]
...
def run_benchmark(backend, model, audio):
    output_file = f"benchmark_official_vulkan_{model['name']}_{audio['name']}.wav"
...
def main():
    print("=== Official Vulkan Version Benchmark ===\n")
    ...
    csv_file = f"benchmark_official_vulkan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
